...
def chkDouble(chkData, dataList, timeList=[]):
    ...
    for i in range(len(dataList)):
        if(chkData==dataList[i]):
            nowTime = time.time()
            #print("DB: {}, Time diff: {}".format(dataList[i], nowTime-timeList[i]))
...
while True:
    ...
    dbTAGS, dbTIMES = scanTAGS(binascii.b2a_hex(data).decode('ascii'))
    print("Found TAGS: {}, TIMES: {}".format(len(dbTAGS), len(dbTIMES)))
    for i in range(0,len(dbTAGS)):
        print ("TAG: {}, Time: {}".format(dbTAGS[i], dbTIMES[i]))
